...
public class Destinyor extends Canvas implements Runnable {
    ...
    Art.setSpritesheet("/icon0.png", 32, 32, this);
    Art.setFont("/8fontTest.png", 8, 8, this);
    Art.setButtons("/button.png", 120, 20, this);
    Art.setMap("/map.png", 512, 512, this);
	...
	private void RenderUI(){
		...
		battle.render(Player.getPlayers(), screen, enemies); // Renders Enemies and Players
	}
...
//				}
			...
			for(NPC n : NPC.npcs.values()) {
	            if(n.inRange() && n.render) {
	            	...
	            	if(n.speaking) {
	            		UI.TextBox(screen);
 	                   	Dialouge.render(screen, n.dialouge, n.dialougeLocation);         
                    }
	...
	private void render() {
			...
			g.drawImage(image, 0, 0, Resolution.width(), Resolution.height(), null);//screen.image, 0, 0, Camera.width, Camera.height, null);
				//g.drawImage(screen.image, 0, 0, Resolution.width(), Resolution.height(), null);
			
			if(UI.menu == 1) {
				battle.renderTime(Player.getPlayers(), g);
			}
