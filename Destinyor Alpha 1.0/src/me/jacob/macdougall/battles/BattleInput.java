	...
	public void assingTarget(Mouse mouse, Enemy[] entities) {
		for(int i = 0; i < entities.length; i++) {
			if(entities.length > 0) {
				if(input.one.down || commands.get(2).inBox(mouse.pressedX, mouse.pressedY)) {
					System.out.println("Targeting");
					entities[i].Focused = false;
					entities[0].Focused = true;
				}
			}
			
			if(entities.length > 1) {
				if(input.two.down || commands.get(3).inBox(mouse.pressedX, mouse.pressedY)) {
					System.out.println("Targeting");
					entities[i].Focused = false;
			...
			if(entities.length > 2) {
				if(input.three.down || commands.get(4).inBox(mouse.pressedX, mouse.pressedY)) {
					System.out.println("Targeting");
					entities[i].Focused = false;
			...
			if(entities.length > 3) {
				if(input.four.down || commands.get(5).inBox(mouse.pressedX, mouse.pressedY)) {
					System.out.println("Targeting");
					entities[i].Focused = false;
			...
			if(entities.length > 4) {
				if(input.five.down || commands.get(6).inBox(mouse.pressedX, mouse.pressedY)) {
					System.out.println("Targeting");
					entities[i].Focused = false;
